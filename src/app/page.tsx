...
import { desc, count, sql, ilike, or, and } from'drizzle-orm';
...
export default async function Dashboard(props: {
 ...
 .from(orders)
 .where(whereClause ? and(whereClause, sql`date_trunc('month', ${orders.createdAt}) = date_trunc('month', CURRENT_DATE)`) : sql`date_trunc('month', ${orders.createdAt}) = date_trunc('month', CURRENT_DATE)`)
 ...
 <h2 className="text-lg md:text-xl font-semibold text-white tracking-tight">Recent Orders</h2>
 <Link href="/orders"className="text-sm font-medium text-white/60 hover:text-white transition-colors whitespace-nowrap">
 View All &rarr;
 </Link>
 </div>
 ...
 <td className="px-4 md:px-8 py-4 md:py-5">
 <span className={`inline-flex items-center px-3 py-1 text-xs font-medium rounded-full border ${order.financialStatus ==='paid'?'bg-white/10 text-white border-white/20':
'bg-neutral-800 text-neutral-300 border-neutral-700'
 ...
 <td className="px-4 md:px-8 py-4 md:py-5">
 <span className={`inline-flex items-center px-3 py-1 text-xs font-medium rounded-full border ${order.fulfillmentStatus ==='fulfilled'?'bg-white/10 text-white border-white/20':
